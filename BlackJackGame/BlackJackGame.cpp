...
Deal additional hands to house
If house is busted
    Everyone who's not busted wins
    Otherwise
        For each player
            If player isn't busted
                If the player's total is greater than the house's total
                    Player wins
...
public:
    ...
    //overloading << operator so I can send Card object to standard output
...
public:
    ...
    void Clear();
    //gets hand total value, intelligently treats all aces as 1 or 11
    int GetTotal() const;
...
int Hand::GetTotal() const
{
    ...
    {
        //add only 10 sunce we've already added 1 for the ace
        total += 10;
    }
...
public:
    ...
    //returns wether generic player has busted - has a total greater than 21
    bool IsBusted() const;
...
public:
    ...
    //returns wether or not the player wants another hit
...
public:
    ...
    virtual ~House();
    //indicates wether house is hitting - will always hit on 16 or less
    virtual bool IsHitting() const;
